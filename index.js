...
// Known subjects and common ways people (or the AI) might phrase them. Checked BEFORE generic
// slugification so "Anatomy and Psysiology" (typo included), "A&P", "anatomy", etc. all land in
// the same "anaphy" bucket instead of fragmenting into separate near-duplicate subjects.
const KNOWN_SUBJECT_ALIASES = {
  anaphy: ['anaphy', 'anatomy and physiology', 'anatomy & physiology', 'anatomy physiology', 'a&p', 'a and p', 'anatomy', 'physiology'],
  biochem: ['biochem', 'biochemistry'],
  nstp: ['nstp', 'national service training', 'national service'],
  tfn: ['tfn', 'theoretical foundation in nursing', 'theoretical foundation', 'foundation in nursing', 'foundation of nursing'],
  understanding_the_self: ['understanding the self', 'understanding_the_self', 'gec 1', 'gec1'],
  philippine_history: ['philippine history', 'readings in the philippine history', 'ip education', 'gec 2a', 'gec2a', 'gec 2'],
  ecs: ['enhanced communication skills', 'enhanced communication'],
  mct: ['movement competency training', 'pathfit', 'mct'],
};

function resolveKnownSubject(rawText) {
...
function slugifySubject(raw) {
  if (!raw) return 'misc';
  const known = resolveKnownSubject(raw);
...
async function handleMessage(text, senderId) {
  ...
  // Catch "link + plain text" (no #hashtag) deterministically — e.g. pasting a link followed by
  // "Anatomy and Physiology" on the next line. Previously this had no dedicated handler: it wasn't
  // a bare URL (extra text present) and had no #tag, so it fell all the way through to the AI
  // intent classifier, which sometimes badly misrouted it (e.g. to check_schedule) instead of
  // adding the link. Since the person already told us exactly what subject they mean, use their
  // words directly instead of asking the AI to guess the whole message's intent.
  if (urlMatch) {
    const url = urlMatch[0];
    const remainingText = trimmed.replace(url, '').trim();
    if (remainingText) {
      return addLink(senderId, remainingText, url);
    }
  }
